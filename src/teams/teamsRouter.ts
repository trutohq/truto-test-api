...
router.get('/', async (c) => {
  ...
  return c.json(await teamsService.list({ 
    cursor, 
    limit,
...
router.post('/', async (c) => {
  ...
  try {
    ...
    if (error instanceof Error && error.message.includes('UNIQUE constraint failed')) {
      throw new HTTPException(409, { message: 'Team with this name already exists in your organization' });
    }
    throw error;
  }
});
...
router.post('/:id/members', async (c) => {
  ...
  if (user.role !== 'admin') {
    throw new HTTPException(403, { message: 'Only admins can add team members' });
  }
  ...
  if (userToAdd && userToAdd.organization_id !== user.organization_id) {
    throw new HTTPException(403, { message: 'Cannot add users from other organizations' });
  }
...
router.delete('/:id/members/:userId', async (c) => {
  ...
  if (user.role !== 'admin') {
    throw new HTTPException(403, { message: 'Only admins can remove team members' });
  }
  ...
  if (userToRemove.organization_id !== user.organization_id) {
    throw new HTTPException(403, { message: 'Cannot remove users from other organizations' });
  }

  const removed = await teamsService.removeMember(id, userId);
  if (!removed) {
    throw new HTTPException(400, { message: 'Failed to remove member from team' });
  }
