...
router.get('/', async (c) => {
  ...
  return c.json(await attachmentsService.list({
    cursor,
    limit,
...
router.post('/', async (c) => {
  ...
  try {
    const attachment = await attachmentsService.saveFile(file, user.organization_id);
...
router.post('/:id/ticket/:ticketId', async (c) => {
  ...
  try {
    ...
    throw new HTTPException(500, { message: 'Failed to add attachment to ticket' });
  }
});
...
router.post('/:id/comment/:commentId', async (c) => {
  ...
  try {
    ...
    throw new HTTPException(500, { message: 'Failed to add attachment to comment' });
  }
});
...
router.delete('/:id/ticket/:ticketId', async (c) => {
  ...
  try {
    ...
    throw new HTTPException(500, { message: 'Failed to remove attachment from ticket' });
  }
});
...
router.delete('/:id/comment/:commentId', async (c) => {
  ...
  try {
    ...
    throw new HTTPException(500, { message: 'Failed to remove attachment from comment' });
  }
});
