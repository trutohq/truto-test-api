...
router.get('/', async (c) => {
  ...
  return c.json(await contactsService.list({ 
    cursor, 
    limit,
...
router.post('/', async (c) => {
  ...
  if ((!data.emails || data.emails.length === 0) && (!data.phones || data.phones.length === 0)) {
    throw new HTTPException(400, { message: 'At least one email or phone number is required' });
  }
...
router.patch('/:id', async (c) => {
  ...
  const willHaveEmail = data.emails ? data.emails.length > 0 : contact.emails.length > 0;
  const willHavePhone = data.phones ? data.phones.length > 0 : contact.phones.length > 0;

  if (!willHaveEmail && !willHavePhone) {
    throw new HTTPException(400, { message: 'Contact must have at least one email or phone number' });
  }
