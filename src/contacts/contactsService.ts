...
export class ContactsService extends BaseService<Contact> {
  ...
  async findExistingContact(organizationId: number, emails: string[] = [], phones: string[] = []): Promise<Contact | undefined> {
  ...
  async list({ cursor, limit = 10, organization_id, email, phone, name }: ListContactsOptions = {}): Promise<PaginatedResponse<Contact>> {
    ...
    if (email) {
      conditions.push('EXISTS (SELECT 1 FROM contact_emails ce WHERE ce.contact_id = c.id AND ce.email LIKE ?)');
      params.push(`%${email}%`);
    }

    if (phone) {
      conditions.push('EXISTS (SELECT 1 FROM contact_phones cp WHERE cp.contact_id = c.id AND cp.phone LIKE ?)');
      params.push(`%${phone}%`);
    }
