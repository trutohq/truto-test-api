...
/**
 * Additive columns layered onto the pre-existing `users` table so it can serve
 * as a User Directory fixture (GWS/M365-style) in addition to a ticketing agent
 * record. These are applied with guarded `ALTER TABLE ... ADD COLUMN` so an
 * already-populated database (e.g. production) keeps every existing row — the
 * new columns simply default to NULL / 'active'.
 */
const USER_DIRECTORY_COLUMNS: Array<{ name: string; definition: string }> = [
  { name: 'first_name', definition: 'TEXT' },
  { name: 'last_name', definition: 'TEXT' },
  { name: 'username', definition: 'TEXT' },
  { name: 'title', definition: 'TEXT' },
  // Directory status. Intentionally has no CHECK constraint (SQLite can't add
...
/**
 * Idempotent schema setup. Safe to run on every boot and via the migrate CLI:
 * `CREATE TABLE IF NOT EXISTS` for tables and guarded `ADD COLUMN` for the
 * directory fields mean existing data is always preserved.
 */
export function runMigrations() {
  ...
  // Layer directory-user columns onto the users table (guarded, additive).
  for (const column of USER_DIRECTORY_COLUMNS) {
    addColumnIfMissing('users', column.name, column.definition)
  }

  // Create user_emails table (a user can expose multiple emails, mirroring
  // contact_emails). The primary email also lives on users.email for the
  // existing ticketing/auth paths.
  db.run(`
  ...
  // Create user_phones table (multiple phones per user).
  db.run(`
  ...
  // Create teams table (also surfaced as directory "groups")
  db.run(`
  ...
  // Create team_members table (user <-> team/group membership)
  db.run(`
